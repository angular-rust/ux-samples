...
impl Application {
    fn new() -> Self {
        ...
        app.window
            ...
            .set_title("UX Framework - Centered text")
            .show()
        ...
        surface.connect_draw(move |_widget, ctx, width, height| {
            ...
            ctx.set_font("Roboto", TextStyle::Normal, TextWeight::Bold, 60.0);
    
            ctx.set_fill_color(color::TEAL_9); // Stroke color
    
            let extents = ctx.measure_text("ZetCode");
    
            let x = (width as f64 - extents.width) / 2.0;
            let y = height as f64 / 2.0;
            ctx.fill_text("ZetCode", x, y);
